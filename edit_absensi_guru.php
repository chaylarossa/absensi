...
// Ambil data absensi dan guru
$stmt = $conn->prepare("
    SELECT ag.*, g.nama, g.nip, g.mapel 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $update = $conn->prepare("UPDATE absensi_guru SET status = ?, tanggal = ? WHERE id = ?");
        $update->bind_param("ssi", $status, $tanggal, $id);
        
        if ($update->execute()) {
            header("Location: absensi_guru.php?status=success&message=Data absensi berhasil diperbarui");
            exit();
        } else {
    ...
    <div class="max-w-2xl w-full bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-[#074799] to-blue-600 p-6">
            <h2 class="text-2xl font-bold text-white flex items-center gap-2">
                <i class="fas fa-edit"></i>Edit Absensi Guru
            </h2>
            <!-- Info Guru -->
        ...
        <div class="p-6">
            ...
            <form method="POST" class="space-y-6">
                ...
                <div class="flex justify-end gap-4">
                    <a href="absensi_guru.php" class="px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">
